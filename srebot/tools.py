...
def _get_robot_file(config, name):
    ...
    if not name:
        ...
        questions = [inquirer.List("file", choices=scripts)]
        answer = inquirer.prompt(questions)
        if not answer["file"]:
            _raise_error("No file selected")
        return answer["file"]

    filtered = list(filter(lambda x: str(name) in x.name, iterate_scripts(config)))
    if len(filtered) > 1:
...
def _select_bot_path():
    ...
    questions = [
        inquirer.List(
            name="path", message="Path", choices=sorted(config.config["bots-paths"])
        ),
    ]
    answer = inquirer.prompt(questions)
    if not answer:
